...
def run_migration():
    ...
    try:
        ...
        admin_user = init_admin(
            db, "admin", "admin123", "admin"
        )  # CORRIGIDO: Usando 'init_admin'
        migration_user = init_admin(
            db, MIGRATION_USER, "nao_usar", "admin"
        )  # CORRIGIDO: Usando 'init_admin'
        ...
        if os.path.exists(clientes_path):
            ...
            for cliente_data in clientes_json:
                try:
                    ...
                    logger.warning(
                        f"Falha ao migrar cliente {cliente_data.get('cpf')}: {e}"
                    )
                except Exception as e:
                    logger.error(
                        f"Erro FATAL ao migrar cliente {cliente_data.get('cpf')}: {e}",
                        exc_info=True,
                    )
        else:
        ...
        if os.path.exists(apolices_path):
            ...
            for apolice_data in apolices_json:
                ...
                if cpf_cliente not in cpf_to_id:
                    logger.warning(
                        f"Apólice N°{apolice_data.get('numero')} ignorada: Cliente {cpf_cliente} não migrado."
                    )
                    continue
                ...
                try:
                    ...
                    # Validação adicional
                    if not apolice_payload.get("premio") or not apolice_payload.get(
                        "tipo_seguro"
                    ):
                        apolice_payload["premio"] = apolice_data.get("valor_premio")
                        apolice_payload["tipo_seguro"] = apolice_data.get(
                            "seguro", {}
                        ).get("tipo")
                    ...
                    logger.info(
                        f"Apólice N°{apolice.numero} migrada (ID={apolice.id})."
                    )

                except BusinessError as e:
                    logger.warning(
                        f"Falha ao migrar apólice {apolice_data.get('numero')}: {e}"
                    )
                except Exception as e:
                    logger.error(
                        f"Erro FATAL ao migrar apólice {apolice_data.get('numero')}: {e}",
                        exc_info=True,
                    )
        else:
        ...
        if os.path.exists(sinistros_path):
            ...
            for sinistro_data in sinistros_json:
                ...
                if numero_apolice not in apolice_num_to_id:
                    logger.warning(
                        f"Sinistro ID={sinistro_data.get('id')} ignorado: Apólice {numero_apolice} não foi migrada ou não existe."
                    )
                    continue
                ...
                try:
                    ...
                    sinistro = create_sinistro(
                        db, sinistro_payload, user=migration_user
                    )
                    logger.info(
                        f"Sinistro migrado (ID={sinistro.id}) para Apólice N°{numero_apolice}."
                    )

                except BusinessError as e:
                    logger.warning(
                        f"Falha ao migrar sinistro da apólice {numero_apolice}: {e}"
                    )
                except Exception as e:
                    logger.error(
                        f"Erro FATAL ao migrar sinistro da apólice {numero_apolice}: {e}",
                        exc_info=True,
                    )
        else:
            logger.warning(
                f"Arquivo de sinistros não encontrado em: {sinistros_path}. Nenhuma migração de sinistros executada."
            )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join("data", "sprint2")):
        ...
        print(
            "POR FAVOR, coloque seus arquivos 'clientes.json', 'apolices.json' e 'sinistros.json' da Sprint 2 nesta pasta."
        )
    else:
        run_migration()
