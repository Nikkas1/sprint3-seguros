...
class SistemaSeguros:
    ...
    def cadastrar_cliente(self):
        ...
        try:
            ...
            print(
                f"Cliente {novo_cliente.nome} cadastrado com sucesso (ID: {novo_cliente.id})!"
            )
    ...
    def atualizar_cliente(self, cpf, telefone=None, email=None, endereco=None):
        ...
        db = next(get_db())
        try:
            cliente_atualizado = dao.update_cliente(
                db, cpf, telefone, email, endereco, self.usuario_logado
            )
    ...
    def emitir_apolice(self, cpf, tipo_seguro, dados_seguro):
        ...
        if tipo_seguro == "1":
            ...
            seguro_obj = SeguroResidencial(
                dados_seguro["endereco"], dados_seguro["valor"]
            )
        elif tipo_seguro == "3":
            seguro_obj = SeguroVida(
                dados_seguro["beneficiarios"], dados_seguro["valor"]
            )
        else:
    ...
    def cancelar_apolice(self, numero):
        ...
        confirmacao = input(
            f"Tem certeza que deseja CANCELAR a Apólice N° {numero}? (s/N): "
        ).lower()
        if confirmacao != "s":
    ...
    def registrar_sinistro(self, numero_apolice, descricao, data):
        ...
        try:
            ...
            print(
                f"Sinistro registrado na Apólice {numero_apolice} (ID: {sinistro.id})!"
            )
            return sinistro
            ...
            self.logger.warning(
                f"Falha ao registrar sinistro na apólice {numero_apolice}: {e}"
            )
    ...
    def atualizar_status_sinistro(self, numero_apolice, novo_status):
        ...
        db = next(get_db())
        try:
            sinistro = dao.update_sinistro_status(
                db, numero_apolice, novo_status, self.usuario_logado
            )
            print(
                f"Status do Sinistro da Apólice {numero_apolice} atualizado para '{sinistro.status}'."
            )

        except BusinessError as e:
            print(f"Erro ao atualizar status: {e}")
            self.logger.warning(
                f"Falha ao atualizar status do sinistro {numero_apolice}: {e}"
            )

        except Exception as e:
            self.logger.error(
                f"Erro ao atualizar status do sinistro: {e}", exc_info=True
            )
            print("Ocorreu um erro interno. Verifique os logs.")
    ...
    def _display_and_export_report(
        self, report_data: list | dict, title: str, filename_prefix: str
    ):
        ...
        if isinstance(report_data, list):
            ...
            if report_data:
                ...
                for row in report_data:
                    for header in headers:
                        ...
                        if isinstance(row.get(header), (int, float)):
                            ...
                            if (
                                header.lower().find("valor") != -1
                                or header.lower().find("premio") != -1
                            ):
                                display_value = (
                                    f"R$ {row.get(header, 0):,.2f}".replace(",", "X")
                                    .replace(".", ",")
                                    .replace("X", ".")
                                )
                                row[header] = row.get(
                                    header
                                )  # Mantém o original para CSV
                            else:
                                display_value = str(row.get(header))
                ...
                # Imprime o cabeçalho
                header_line = " | ".join(
                    f"{h.upper():<{col_widths[h]}}" for h in headers
                )
                print(header_line)
                ...
                for row in report_data:
                    ...
                    for h in headers:
                        ...
                        if isinstance(value, (int, float)):
                            if (
                                h.lower().find("valor") != -1
                                or h.lower().find("premio") != -1
                            ):
                                display_value = (
        ...
        if confirmacao == "s":
            if isinstance(report_data, list) and report_data:
                filepath = export_to_csv(
                    report_data, filename_prefix, self.usuario_logado.username
                )
                if filepath:
    ...
    def relatorio_valor_segurado_por_cliente(self):
        ...
        try:
            ...
            self.logger.error(
                f"Erro ao gerar relatório de valor segurado: {e}", exc_info=True
            )
    ...
    def relatorio_apolices_por_tipo(self):
        ...
        try:
            ...
            self.logger.error(
                f"Erro ao gerar relatório de apólices por tipo: {e}", exc_info=True
            )
            print("Erro ao gerar relatório. Verifique os logs.")
        finally:
            db.close()
    ...
    def relatorio_sinistros_status(self):
        ...
        try:
            ...
            self._display_and_export_report(
                dados, "Status dos Sinistros (Contagem)", "sinistros_por_status"
            )
        except Exception as e:
            self.logger.error(
                f"Erro ao gerar relatório de sinistros por status: {e}", exc_info=True
            )
            print("Erro ao gerar relatório. Verifique os logs.")
        finally:
            db.close()
    ...
    def relatorio_ranking_clientes(self):
        ...
        try:
            ...
            self.logger.error(
                f"Erro ao gerar relatório de ranking de clientes: {e}", exc_info=True
            )
            print("Erro ao gerar relatório. Verifique os logs.")
        finally:
            db.close()
