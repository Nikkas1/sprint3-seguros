...
from .exceptions import ApoliceInexistente, BusinessError, CpfInvalido, LoginInvalido
from .logger import get_logger
from .models import Apolice, Audit, Cliente, Sinistro, User
from .utils import (
    gerar_numero_apolice,
    hash_password,
    parse_date,
    validar_cpf,
    verify_password,
...
def update_cliente(
    ...
    try:
        ...
        if not updates:
            logger.info(
                f"Cliente CPF={cpf_limpo} encontrado, mas nenhum dado foi alterado."
            )
            return cliente
        ...
        db.commit()
        logger.info(
            f"Cliente CPF={cpf_limpo} atualizado. Campos alterados: {list(updates.keys())}"
        )
...
def create_apolice(db: Session, data: dict, user: User):
    ...
    try:
        ...
        db.commit()
        logger.info(
            f"Apólice N°{apolice.numero} criada para o Cliente ID={cliente.id}."
        )
...
def get_apolice_by_numero(db: Session, numero: str):
    ...
    try:
        ...
        get_logger("system", "ERROR").error(
            f"Erro ao desserializar dados_seguro da Apólice N°{numero}. Conteúdo: {apolice.dados_seguro}"
        )
        apolice.dados_seguro = {}
...
def create_sinistro(db: Session, data: dict, user: User):
    ...
    try:
        ...
        if apolice.status != "Ativa":
            raise BusinessError(
                f"Apólice N°{numero_apolice} não está ativa (Status: {apolice.status})."
            )
...
def update_sinistro_status(
    db: Session, numero_apolice: str, novo_status: str, user: User
):
    ...
    if novo_status not in STATUS_PERMITIDOS:
        raise BusinessError(
            f"Status inválido: {novo_status}. Use: {', '.join(STATUS_PERMITIDOS)}."
        )
    ...
    try:
        ...
        if not sinistro:
            raise BusinessError(
                f"Sinistro não encontrado para a Apólice N°{numero_apolice}."
            )
        ...
        db.commit()
        logger.info(
            f"Status do Sinistro ID={sinistro.id} atualizado para '{novo_status}'."
        )
        ...
        db.rollback()
        logger.error(
            f"Erro inesperado ao atualizar Sinistro da Apólice N°{numero_apolice}",
            exc_info=True,
        )
        raise BusinessError(
            "Erro interno ao atualizar o status do sinistro. Consulte logs."
        )
...
def relatorio_valor_segurado_por_cliente(db: Session):
    query = (
        ...
        .join(Apolice, Cliente.id == Apolice.cliente_id)
        .where(Apolice.status == "Ativa")
        .group_by(Cliente.id, Cliente.nome, Cliente.cpf)
    )
    ...
    relatorio = [
        {
            "nome": r.nome,
            "cpf": r.cpf,
            "valor_total_segurado": float(r.valor_total_segurado or 0),
        }
        for r in resultados
    ]
    return relatorio
...
def relatorio_apolices_por_tipo(db: Session):
    query = (
        select(Apolice.tipo, func.count(Apolice.id).label("quantidade"))
        .where(Apolice.status == "Ativa")
        .group_by(Apolice.tipo)
    )
...
def relatorio_sinistros_status(db: Session):
    query = select(
        Sinistro.status, func.count(Sinistro.id).label("quantidade")
...
def relatorio_ranking_clientes(db: Session):
    query = (
        ...
        .where(Apolice.status == "Ativa")
        .group_by(Cliente.id, Cliente.nome, Cliente.cpf)
        .order_by(desc("total_apolices_ativas"))
    )
